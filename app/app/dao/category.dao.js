...
Category.create = (newCategory, done) => {
  const date = new Date(newCategory.category_creation_date);
  if (isNaN(date.getTime())) {
    return done({ error: "incorrect date value" });
  } else {
    newCategory.category_creation_date = date.toISOString().split("T")[0];
  }
...
Category.updateById = (id, upCategory, done) => {
  ...
  const date = new Date(upCategory.category_creation_date);
