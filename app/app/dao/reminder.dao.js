...
Reminder.create = (newReminder, done) => {
  const date = new Date(newReminder.reminder_creation_date);
  if (isNaN(date.getTime())) {
    return done({ error: "incorrect date value" });
  } else {
    newReminder.reminder_creation_date = date.toISOString().split("T")[0];
  }
...
Reminder.updateById = (id, upReminder, done) => {
  const date = new Date(upReminder.reminder_creation_date);
  if (isNaN(date.getTime())) {
    return done({ error: "incorrect date value" });
  }
